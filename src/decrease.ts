...
import { getLeverage, getNetPriceImpactDeltaUsdForDecrease, getPositionPnlUsd } from "./sdk/utils/positions.js";
...
export function getDecreasePositionAmountsOrig(p: {
    marketInfo: MarketInfo;
    collateralToken: TokenData;
    isLong: boolean;
    ...
    if (!position || position.sizeInUsd <= 0 || position.sizeInTokens <= 0) {
        ...
        const positionFeeInfo = getPositionFee(
            marketInfo,
            values.sizeDeltaUsd,
            values.positionPriceImpactDeltaUsd > 0n,
            userReferralInfo
        );
...
export function getIsFullCloseOrig(p: {
    position: PositionInfo;
    sizeDeltaUsd: bigint;
    indexPrice: bigint;
    remainingCollateralUsd: bigint;
...
function applyAcceptablePriceOrig(p: {
    marketInfo: MarketInfo;
    isLong: boolean;
    isTrigger: boolean;
    fixedAcceptablePriceImpactBps?: bigint;
